...
router.get("/:ip", (req, res) => {
    ...
    if ((0, ip_utils_1.TestIPv4Address)(ipAddress)) {
        ...
        // get the cloud provider subnets (and region/service), filtered on the first octet of the IP Address
        var CloudProviderDetails = (0, cloudprovider_utils_1.GetCloudProviderSubnets)('./src/cloudproviders/Azure.json', ipAddress.split(".")[0]);
        // CloudProviderDetails += GetCloudProviderSubnets('./src/cloudproviders/AWS.json', ipAddress.split(".")[0]);
        // CloudProviderDetails += GetCloudProviderSubnets('./src/cloudproviders/Azure.json', ipAddress.split(".")[0]);
        // CloudProviderDetails += GetCloudProviderSubnets('./src/cloudproviders/GoogleCloud.json', ipAddress.split(".")[0]);
        // CloudProviderDetails += GetCloudProviderSubnets('./src/cloudproviders/OCI.json', ipAddress.split(".")[0]);
        // CloudProviderDetails += GetCloudProviderSubnets('./src/cloudproviders/Akamai.json', ipAddress.split(".")[0]);
        // CloudProviderDetails += GetCloudProviderSubnets('./src/cloudproviders/CloudFlare.json', ipAddress.split(".")[0]);
